...
export default async function MerchantOrderDetailPage({
  ...
  // Duty status only gates automatic assignment (see findRiderForAutoAssignment
  // in src/lib/autoAssign.ts) -- manual assignment is a human deliberately
  // picking a rider, so every active rider is a candidate here regardless of
  // whether they've checked into their duty link.
  ...
  // tracking_tokens has no merchant RLS policy (service-role-only resource)
  // -- safe to read via service-role here specifically because `order` was
  // only reachable above through the RLS-authenticated client, which
  // already proved it belongs to this merchant's own tenant.
